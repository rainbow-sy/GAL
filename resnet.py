...
class SparseResBasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1):
        ...
        self.stride = stride
        self.shortcut = nn.Sequential()
        if stride != 1 or inplanes != planes:#对于通道不一致或者stride为2时hw减半的情况
            self.shortcut = LambdaLayer(lambda x: F.pad(x[:, :, ::2, ::2], (0, 0, 0, 0, planes//4, planes//4), "constant", 0))
            #：：2代表切片start:stop:step,对于图像就是进行下采样，图像的hw变为原来的1/2
        m = Normal(torch.tensor([norm_mean]), torch.tensor([norm_var])).sample()#对正态分布(norm_mean,norm_var)随机采样一个数
        self.mask = Mask(m)
...
if __name__=='__main__':
    ...
    model=resnet_56_sparse(num_classes=4)
    mask = []
    for name, param in model.named_parameters():
        if 'mask' in name:
            mask.append(param.view(-1))  # param.view(-1),将param展平成一维。
    mask = torch.cat(mask)
    output=model(input)